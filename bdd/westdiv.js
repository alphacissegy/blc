...
async function transferFromWestdiv() {
  const client = await pool.connect();
  try {
    const res = await client.query(`SELECT * FROM westdiv`);
    const rows = res.rows;
    let inserted = 0;
    ...
    for (const row of rows) {
      ...
      // Vérifie si la ligne contient autre chose que les valeurs par défaut
      const hasNonDefault = [
    ...
    console.log(`✅ ${inserted} ligne(s) valides copiée(s) de westdiv → allstars_divs_fiches`);
  } catch (error) {
    console.error("❌ Erreur transfert westdiv → allstars_divs_fiches :", error);
  } finally {
    client.release();
  }
}

// Exécute la création puis le transfert
createAllStarsDivsFichesTable().then(() => {
  transferFromWestdiv();
});
